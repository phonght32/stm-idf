...
uint32_t RCC_AHBENR_ADCEN_MAPPING[ADC_NUM_MAX] = {
	RCC_AHB2ENR_ADCEN,
	RCC_AHB2ENR_ADCEN,
	RCC_AHB2ENR_ADCEN,
};
...
stm_err_t adc_config(adc_cfg_t *config)
{
    ...
    SET_BIT(RCC->AHB2ENR, RCC_AHBENR_ADCEN_MAPPING[config->adc_num]);
    tmpreg = READ_BIT(RCC->AHB2ENR, RCC_AHBENR_ADCEN_MAPPING[config->adc_num]);
    UNUSED(tmpreg); 
